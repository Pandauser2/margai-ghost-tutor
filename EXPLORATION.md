...
**Conclusion:** Pilot: **manual PDF upload** (reuse upsc-test-engine upload), n8n workflows, Supabase (`institutes`, `uploads`, `query_logs`), Pinecone. Contact info (`email_for_report`, `ta_whatsapp_number`) in `institutes` or sheet. No "read sheet → download PDF".
...
| **Pinecone serverless** | One namespace per institute = slug (e.g. `fiitjee-kolkata`). Create index with dimension matching embedding model (e.g. **768** for `text-embedding-004` or Gemini embedding). | Namespace created on first upsert. Institute slug and contact info live in Supabase `institutes` table. |
...
- **Workflow 2 – WhatsApp:** Webhook (verify signature) → Code (parse body; if incoming text is "escalate" and there is a recent row for this student with `clarification_sent = true`, treat as escalate that row → Supabase update + reply to student + notify TA) → else insert Supabase → Embeddings Google Gemini (query text) → Pinecone query → Code (build prompt) → HTTP Gemini generateContent (**gemini-2.5-flash**) → IF response === ESCALATE → send **clarifying question** to student (do not escalate yet; set `clarification_sent = true`); ELSE IF student previously asked to escalate → reply "We've escalated…" + get TA from `institutes` + WhatsApp to TA with full message; ELSE → WhatsApp send answer to student.
...
- **Phase 2 – WhatsApp + RAG:** Webhook workflow (verify signature): receive message → log to Supabase → Pinecone retrieval → Gemini → reply or escalate (reply to student on escalate, then TA from `institutes` or sheet). Manual test with text and photo.
